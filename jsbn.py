...
# Code follows:
var.registers(['barrettConvert', 'bnpInvDigit', 'bnRemainder', 'bnDivideAndRemainder', 'bnAbs', 'barrettSqrTo', 'am3', 'bnGCD', 'bnOr', 'am2', 'int2char', 'bnSigNum', 'bnShiftRight', 'BI_FP', 'cConvert', 'rr', 'bnAndNot', 'bnAnd', 'montReduce', 'bnSubtract', 'bnpMultiplyTo', 'intAt', 'bnpDLShiftTo', 'barrettMulTo', 'BI_RM', 'bnpModInt', 'lbit', 'bnClone', 'cReduce', 'bnModPowInt', 'bnDivide', 'bnMax', 'bnCompareTo', 'nNop', 'montConvert', 'op_and', 'bnpClamp', 'op_xor', 'bnpExp', 'bnIntValue', 'vv', 'bnpFromRadix', 'montSqrTo', 'bnFlipBit', 'bnSetBit', 'bnpSquareTo', 'nbits', 'bnTestBit', 'bnXor', 'bnNot', 'bnModInverse', 'Montgomery', 'dbits', 'bnpFromInt', 'bnpFromNumber', 'cSqrTo', 'bnpRShiftTo', 'bnpDAddOffset', 'bnpCopyTo', 'bnpDivRemTo', 'nMulTo', 'montRevert', 'cRevert', 'montMulTo', 'Barrett', 'nbv', 'NullExp', 'bnPow', 'bnpIsEven', 'cbit', 'j_lm', 'bnpAddTo', 'bnpBitwiseTo', 'bnMin', 'bnpSubTo', 'bnEquals', 'am1', 'bnIsProbablePrime', 'bnShiftLeft', 'bnAdd', 'nbi', 'bnShortValue', 'nSqrTo', 'bnMultiply', 'lowprimes', 'bnToString', 'BI_RC', 'canary', 'bnpDRShiftTo', 'cMulTo', 'bnpChunkSize', 'bnMod', 'bnpFromString', 'bnModPow', 'op_or', 'bnpDMultiply', 'BigInteger', 'Classic', 'barrettRevert', 'bnBitCount', 'bnpChangeBit', 'bnpToRadix', 'bnNegate', 'bnByteValue', 'bnClearBit', 'barrettReduce', 'bnpMultiplyUpperTo', 'op_andnot', 'lplim', 'bnpMultiplyLowerTo', 'bnBitLength', 'bnGetLowestSetBit', 'bnToByteArray', 'bnpLShiftTo', 'bnpMillerRabin'])
@Js
...
def PyJsHoisted_am2_(i, x, w, j, c, n, this, arguments, var=var):
    var = Scope({'i':i, 'x':x, 'w':w, 'j':j, 'c':c, 'n':n, 'this':this, 'arguments':arguments}, var)
    var.registers(['xh', 'xl', 'm', 'w', 'n', 'i', 'x', 'j', 'c', 'l', 'h'])
...
def PyJsHoisted_bnToString_(b, this, arguments, var=var):
    ...
    var.registers(['m', 'd', 'p', 'r', 'b', 'i', 'k', 'km'])
...
def PyJsHoisted_bnpLShiftTo_(n, r, this, arguments, var=var):
    var = Scope({'n':n, 'r':r, 'this':this, 'arguments':arguments}, var)
    var.registers(['bs', 'ds', 'cbs', 'n', 'r', 'c', 'i', 'bm'])
...
def PyJsHoisted_bnpRShiftTo_(n, r, this, arguments, var=var):
    var = Scope({'n':n, 'r':r, 'this':this, 'arguments':arguments}, var)
    var.registers(['bs', 'ds', 'cbs', 'n', 'r', 'i', 'bm'])
    var.get('r').put('s', var.get(u"this").get('s'))
    var.put('ds', var.get('Math').callprop('floor', (var.get('n')/var.get(u"this").get('DB'))))
...
def PyJsHoisted_bnpDivRemTo_(m, q, r, this, arguments, var=var):
    ...
    var.registers(['pt', 'yt', 'ys', 'y', 'y0', 'm', 't', 'r', 'nsh', 'd2', 'e', 'j', 'pm', 'd1', 'ms', 'ts', 'qd', 'q', 'i'])
...
def PyJsHoisted_bnpFromRadix_(s, b, this, arguments, var=var):
    var = Scope({'s':s, 'b':b, 'this':this, 'arguments':arguments}, var)
    var.registers(['cs', 'w', 'd', 'b', 'j', 'i', 'x', 'mi', 's'])
...
def PyJsHoisted_bnModPow_(e, m, this, arguments, var=var):
    var = Scope({'e':e, 'm':m, 'this':this, 'arguments':arguments}, var)
    var.registers(['g2', 'm', 'is1', 'w', 'z', 'k1', 't', 'e', 'r', 'n', 'i', 'j', 'r2', 'k', 'km', 'g'])
...
def PyJsHoisted_bnModInverse_(m, this, arguments, var=var):
    var = Scope({'m':m, 'this':this, 'arguments':arguments}, var)
    var.registers(['m', 'd', 'b', 'v', 'a', 'c', 'u', 'ac'])
...
def PyJsHoisted_bnpMillerRabin_(t, this, arguments, var=var):
    var = Scope({'t':t, 'this':this, 'arguments':arguments}, var)
    var.registers(['t', 'r', 'n1', 'i', 'a', 'y', 'j', 'k'])
...
def customModPowIntCall(password, mod, exp, this, arguments, var=var):
    var = Scope({'$data': password, 'modulus': mod, 'encryptionExponent': exp, 'this': this, 'arguments': arguments}, var)
    var.registers(['$modulus', '$exp', '$data'])
    var.put('$data', var.get('$data').callprop('modPowInt', var.get('encryptionExponent'), var.get('modulus')))
    print(var.get('$data'))

def run(password, mod, exp):
    var.put('RSA', Js({'customModPowInt':customModPowIntCall}))
    print(var.get('RSA').callprop('customModPowInt', password, mod, exp))
